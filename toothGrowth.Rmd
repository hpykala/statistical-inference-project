...
We use ToothGrowth dataset from the R library datasets. Descripion of the data: The response is the length of odontoblasts (cells responsible for tooth growth) in 60 guinea pigs. Each animal received one of three dose levels of vitamin C (0.5, 1, and 2 mg/day) by one of two delivery methods, (orange juice or ascorbic acid (a form of vitamin C and coded as VC).
...
ggplot(data = tooth, aes(x = dose,y = len)) +
 geom_boxplot(fill="steelblue") +
    ...
    ggtitle("Tooth length by supplement method and dose") +
    xlab("Dose") + ylab("Odontoblast length")
    
```

From the boxplot it seems clear that the dosage of the vitamin C has correlation with the tooth growth. The supplement type doesn't have such a clear connection. 
...
the p-value of the test is `r round(pt1$p,2)` We can't reject the null hypothesis on 0.05 signifigance level.
...
```
The p-value is close to zero so we can reject the null hypothesis on 0.05 signifigance level or any other reasonable critical value. The difference between groups is `r round(pt2$origStat,2)` so we can conclude that increasing the dosage from 0.5 to 1.0 increases the tooth growth.

```{r}
pt3 <- permTest(tooth$len,tooth$dose, 1.0, 2.0)
data.frame("Difference of means" =  pt3$origStat, "p-value" = pt3$p)
```
The p-value is close to zero so we can reject the null on 0.05 signifigance level. The difference between groups is `r round(pt3$origStat,2)` so we can conclude that increasing the dosage from 1.0 to 2.0 increases the tooth growth.
